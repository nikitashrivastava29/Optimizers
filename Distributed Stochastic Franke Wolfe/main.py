...
import torch.nn.functional as F
import math
from constraints import Constraint, create_lp_constraints, create_k_sparse_constraints
from optimizers import SFW
...
def load_train_objs():
    ...
    model= SmallNet()
    #################################################### CALL THE OPTIMIZER #####################################################################
    optimizer = SFW(model.parameters(),learning_rate=0.1, momentum=0.9)
...
class Trainer:
    def __init__(
        ...
        ############################################# DEFINE CONSTRAINTS ##########################################################################
        ''' This step was missing earlier, now it will use this function from constraints.py file and load our constraints to be passed to the optimizer'''
        '''this is an example of create_lp_constraints, you can use create_k_sparse_constarints fucntion also, need to check paper'''
        self.constraints = create_lp_constraints(self.model, ord=2, value=300, mode='initialization')
    ...
    def _run_batch(self, source, targets):
        ...
        self.optimizer.step(self.constraints)
